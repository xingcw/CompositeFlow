"""Reference min/max returns for normalised scores.

Copied verbatim from ``envs/infos.py``; it is pure data.

These were measured with mujoco-py 2.1. This port runs mujoco 3.10, whose
trajectories differ slightly, so normalised scores are comparable to the
published numbers only up to that engine change. See docs/BACKEND.md.
"""
